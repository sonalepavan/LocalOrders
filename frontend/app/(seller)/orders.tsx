...
export default function SellerOrders() {
  ...
  const load = useCallback(async () => {
    if (!user) return;
    if (!online) {
      const cached = await offlineCache.load<OrderWithBuyer[]>(
        CACHE_SCOPES.sellerOrders,
        user.userId,
        [],
      );
      setOrders(cached);
    ...
    try {
      ...
      setOrders(list);
      setFromCache(false);
      await offlineCache.save(CACHE_SCOPES.sellerOrders, user.userId, list);
    } catch {
      const cached = await offlineCache.load<OrderWithBuyer[]>(
        CACHE_SCOPES.sellerOrders,
        user.userId,
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.background }} edges={["top"]}>
      <Appbar.Header mode="small" elevated>
        ...
        <NotificationBell testID="seller-orders-bell" />
      </Appbar.Header>
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              ...
              <Text variant="bodyMedium" style={{ color: theme.colors.onSurfaceVariant, marginTop: 4, textAlign: "center", paddingHorizontal: 32 }}>
