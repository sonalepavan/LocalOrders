...
type Row = CustomRequest & { buyer: BuyerSummary | null };
...
// Active vs History grouping (seller view — SAVED drafts never reach the seller)
const ACTIVE_STATUSES: CustomRequestStatus[] = ["NEW_REQUEST", "QUOTE_SENT", "ACCEPTED"];
...
export default function SellerCustomRequests() {
  ...
  const load = useCallback(async () => {
    try {
      const { requests } = await api.listSellerCustomRequests();
      setRows(requests);
    } catch (e: any) {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.background }} edges={["top"]}>
      <Appbar.Header mode="small" elevated>
        ...
        <Appbar.Content title="Custom Requests" />
        <NotificationBell testID="seller-custom-requests-bell" />
      </Appbar.Header>
          ...
          ListHeaderComponent={
            <View style={{ marginBottom: 12 }}>
              <SegmentedButtons
                ...
                buttons={[
                  { value: "active", label: "Active", testID: "seller-tab-active" },
                  { value: "history", label: "History", testID: "seller-tab-history" },
                ]}
              />
            </View>
          }
          ListEmptyComponent={
            <View style={styles.empty}>
              <Text variant="titleMedium" style={{ marginBottom: 4 }}>
                {tab === "active" ? "No active custom requests" : "No history yet"}
              </Text>
              <Text
                variant="bodyMedium"
                style={{
              ...
              >
                {tab === "active"
                  ? "Buyer custom requests will appear here."
          ...
          renderItem={({ item }) => {
            ...
            return (
              <Card
                ...
                onPress={() =>
                  router.push({
                    pathname: "/seller-custom-request-detail",
                    params: { requestId: item.requestId },
                  })
                }
                testID={`seller-custom-request-${item.requestId}`}
              >
                <Card.Content>
                  <View style={styles.row}>
                    ...
                    <Chip
                      ...
                      testID={`seller-custom-request-status-${item.requestId}`}
                    >
                      {c.label}
                    </Chip>
                  </View>
